...
- Member ordering: constants, enum cases, constructor, static methods, instance
  ...
  tie-breaker. Enum cases are exempt from the intra-group order, which is
  author-chosen per rule 6 "evident structure" (a registry such as ISO 3166 keeps
  its canonical order). Their group position is still enforced. PHPUnit test classes
  instead order methods as lifecycle hooks (in execution order), then other methods,
  then data providers.
...
def within_group_violations(path: str, declared: TypeMembers) -> tuple[Violation, ...]:
    ...
    Enum cases are exempt: their order is author-chosen (rule 6 "evident structure"),
    so a domain registry such as ISO 3166 may keep its canonical order. Constant and
    method order stays enforced, as does the const, case, method group sequence.
    """
    violations = []
